...
from FSPS_calculations import SEDInterpolator, FSPS_initializer, reshape_coeff_grid, SEDInterpolator_rect
...
# Use the rectangular
tage_grid_rect, logzsol_grid_rect, coeff_grid_rect = reshape_coeff_grid(param_grid, PCA_coeffs)
coeff_new_rect = SEDInterpolator_rect(tage_grid_rect, logzsol_grid_rect, coeff_grid_rect, tage_new, logzsol_new)
...
n_component_recon = 100
SED_recon = reconstruct_PCA_SED(coeff_new, PCA_PCs, n_components = n_component_recon)
SED_recon_rect = reconstruct_PCA_SED(coeff_new_rect, PCA_PCs, n_components = n_component_recon)

SED_recon_denorm = denormalise_SED(SED_recon, SED_norm_factor, SED_norm_mean)
SED_recon_rect_denorm = denormalise_SED(SED_recon_rect, SED_norm_factor, SED_norm_mean)
...
plot_SED(ax[0], wave_masked, SED_recon_denorm, color = 'green', label = 'Delaunay', lw = 2)
plot_SED(ax[0], wave_masked, SED_recon_rect_denorm, color = 'red', label = 'Bilinear', lw = 2)
plot_SED(ax[0], wave_FSPS, SED_FSPS, color = 'grey', lw = 3)

plot_SED(ax[1], wave_masked, SED_recon_denorm/SED_FSPS_masked, color = 'green', lw = 3)
plot_SED(ax[1], wave_masked, SED_recon_rect_denorm/SED_FSPS_masked, color = 'red', lw = 3)
...
ax[0].set_title('SSP SED interpolation (tage: {:.3e} Gyr, logzsol: {:.2f})'.format(tage_new, logzsol_new), fontsize =
                15)
ax[0].set_ylabel('F_lambda', fontsize = 15)
